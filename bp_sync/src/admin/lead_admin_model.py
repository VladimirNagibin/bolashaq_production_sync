...
class LeadAdmin(
    BaseAdmin, AdminListAndDetailMixin, model=Lead
    ...
    async def get_object_for_details(self, request: Request) -> Any:
        ...
        try:
            lead_uuid = uuid.UUID(pk)
        except ValueError:
            raise HTTPException(status_code=404, detail="Неверный формат UUID")
        stmt = (
            select(Lead)
            .options(
                selectinload(Lead.communications).selectinload(
                ...
                selectinload(Lead.created_user),
                selectinload(Lead.modify_user),
                selectinload(Lead.last_activity_user),
            )
    ...
    @staticmethod
    def _format_revenue(model: Lead, attribute: str) -> str:
