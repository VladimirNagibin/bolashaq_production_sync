...
class LeadClient(BaseEntityClient[LeadDB, LeadRepository, LeadBitrixClient]):
    ...
    async def send_overdue_leads_notifications(self) -> None:
        ...
        try:
            ...
            logger.error(
                f"Failed to send overdue leads notifications: {e}",
                exc_info=True,
            )
    ...
    async def _prepare_overdue_leads_notifications(
        ...
        try:
            leads_by_user, critical_leads = (
                await self._group_overdue_leads_by_user(overdue_leads)
            )
            ...
            logger.error(
                f"Failed to prepare notifications: {e}", exc_info=True
            )
            return [
                (
                    SUPERVISOR_ID,
                    "Ошибка при формировании списка просроченных лидов.",
                )
            ]
    ...
    async def _group_overdue_leads_by_user(
        ...
        for lead, overdue_delta in overdue_leads:
            try:
                ...
                if (
                    lead_info["overdue_days"]
                    > SUPERVISOR_OVERDUE_THRESHOLD_DAYS
                ):
                ...
                logger.error(
                    ...
                    f"{getattr(lead, 'external_id', 'unknown')}: {e}",
                    exc_info=True,
                )
                continue
    ...
    async def _extract_lead_notification_info(
        self, lead: LeadDB, overdue_delta: timedelta
    ) -> dict[str, Any]:
        """
        ...
        """
        user_name = getattr(
            lead.assigned_user, "full_name", "Неизвестный пользователь"
        )
        ...
        display_days = (
            max(overdue_days - 1, MIN_OVERDUE_DAYS_FOR_DISPLAY)
            if overdue_days > 1
            else 1
        )
        ...
        return {
            ...
            "status_display": MAPPED_STATUSES.get(
                lead.status_id, "Не определено"
            ),
            "link": lead_link,
    ...
    def _format_supervisor_message(
        self, critical_leads: dict[tuple[str, int], list[dict[str, Any]]]
    ) -> str:
        """
        ...
        for (user_name, _), user_leads in critical_leads.items():
            if user_leads:
        ...
        return (
            "\n".join(supervisor_parts)
            if supervisor_parts
            else "Нет критически просроченных лидов."
        )
